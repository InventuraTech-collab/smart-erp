...
const F = ({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) => (
  <div style={{ marginBottom: 12 }}>
    <label style={{ display:'block', fontSize:11, fontWeight:600, color:'#666', marginBottom:5, textTransform:'uppercase', letterSpacing:.4 }}>
      {label}
    </label>
    {children}
...
export default function LoginPage() {
  ...
  function handleLogin(e: React.FormEvent) {
    e.preventDefault()
    if (!form.company || !form.user || !form.password) {
      setError('Please fill in Company, Username, and Password.')
      return
    }
    setError('')
    setLoading(true)
    setTimeout(() => {
      localStorage.setItem('erp_user', JSON.stringify({
        ...
        branch: form.branch || 'Main Branch — Muscat',
        user: form.user,
    ...
    <div style={{ display:'flex', minHeight:'100vh', fontFamily:'Segoe UI,system-ui,sans-serif' }}>
      ...
      <div style={{ width:'50%', background:'#8B1A1A', display:'flex', flexDirection:'column', justifyContent:'space-between', padding:'2.5rem', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:2 }}>
          ...
          <h1 style={{ fontSize:30, fontWeight:700, color:'#fff', lineHeight:1.25, marginBottom:'1rem', letterSpacing:-.5 }}>
            Inventory &<br/>Business Management
          </h1>
          <p style={{ fontSize:14, color:'rgba(255,255,255,.72)', lineHeight:1.65, maxWidth:320, marginBottom:'2rem' }}>
            A complete ERP suite for managing inventory, purchasing, sales and finance — built for Oman &amp; GCC markets.
          </p>
        ...
        <div style={{ position:'relative', zIndex:2, fontSize:11, color:'rgba(255,255,255,.4)' }}>
          © 2026 Smart ERP · Powered by AIS · All rights reserved
        </div>
      </div>
      ...
      <div style={{ width:'50%', background:'#fff', display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem 3rem' }}>
        <div style={{ width:'100%', maxWidth:400 }}>
          ...
          {error && (
            <div style={{ background:'#fdf2f2', border:'1px solid #f5c6c6', color:'#a32d2d', borderRadius:7, padding:'8px 12px', fontSize:12, marginBottom:14 }}>
              ⚠ {error}
            </div>
          )}
          ...
          <a href="#" style={{ display:'block', textAlign:'center', fontSize:12.5, color:'#F47920', marginTop:12 }}>
            Forgot password?
          </a>
        </div>
      </div>
